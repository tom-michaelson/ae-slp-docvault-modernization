...
# Review Implementation Plan - API

Review a JIT-generated implementation plan for API endpoints, validating quality, accuracy, completeness, and adherence to target architecture standards.
...
```
/develop.review-plan-api key: [key]
```

**Examples:**
```
/develop.review-plan-api key: 1075-spring-filterservice-load
```

```
/develop.review-plan-api key: 3003-spring-companycontactservice-getcompanycontactlist
```
...
The implementation plan must design MODERN RESTful APIs. Flag and correct any of these legacy patterns:
...
#### Consistency:
- Models in OpenAPI match models in implementation-plan.md
...
#### Authorization:
- [ ] Security decision documented: does legacy enforce API-level permission checks?
- [ ] If yes: `@SecuredByMenuItem` on controller with correct item_id documented
- [ ] If no: explicitly stated that no API-level security needed (matches legacy)
- [ ] **DEFECT CHECK:** Plan does NOT contain `hasPermission()` calls in service layer
- [ ] **DEFECT CHECK:** Plan does NOT contain hardcoded func_id constants in services
...
### API-7. Stored Procedure Conversion Compliance (when applicable)

**Goal**: When the API endpoint depends on stored procedures, ensure the conversion plan meets all SP-specific requirements beyond the common review checks (5a).

**Verify API-specific SP integration**:

- [ ] Service Layer section specifies how SP business logic maps to Java service methods (not just describing the SP)
- [ ] Data Access Layer specifies the query approach (derived queries, Specifications, `@Query`, streaming) per v9 decision flowchart
- [ ] Component diagram shows: `Controller → EndpointService → SpConversionService → Repository → Database`
- [ ] Conversion doc planned at `docs/conversions/{sp_name}_conversion.md`
- [ ] Code traceability comment planned: `Replaces stored procedure: {sp_name}.sp` in service JavaDoc

**ACTION**: If SP conversion is treated as an afterthought (no structured strategy section, no complexity score, no conversion doc path), flag as HIGH priority and add the missing content.
...
- [ ] Consistent with implementation-plan.md
...
5. **Make corrections directly** to implementation-plan.md and api.openapi.json as you find issues
...
5. Update both implementation-plan.md and api.openapi.json
6. Document the transformation in review report

### OpenAPI Inconsistencies

**Problem**: OpenAPI spec doesn't match implementation plan

**Solutions**:
1. Use implementation-plan.md as source of truth for design
