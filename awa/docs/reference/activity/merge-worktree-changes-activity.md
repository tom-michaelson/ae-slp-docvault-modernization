# `awa-merge-worktree-changes`

Merge changes from isolated environment back to the original source branch (for ACT mode).

This activity commits any changes in the isolated environment and merges them back to the
original source branch, using the agent provider information for commit attribution.

## Parameters

| Name               | Type                           | Description                                                                |
| :----------------- | :----------------------------- | :------------------------------------------------------------------------- |
| `environment_info` | `IsolatedAgentEnvironmentInfo` | Information about the isolated environment containing changes              |
| `agent_config`     | `AgentConfiguration`           | Agent configuration containing provider information for commit attribution |
...
const { merge_worktree_changes_activity } = proxyActivities<typeof activities>({
...
export async function myWorkflow(
  ...
  return await merge_worktree_changes_activity(environmentInfo, agentConfig);
}
```
...
public class MyWorkflowImpl implements MyWorkflow {
    ...
    public Object run(Object environmentInfo, Object agentConfig) {
        return activities.mergeWorktreeChangesActivity(environmentInfo, agentConfig);
    }
}
```
...
func MyWorkflow(ctx workflow.Context, environmentInfo interface{}, agentConfig interface{}) (interface{}, error) {
	...
	err := workflow.ExecuteActivity(ctx, "merge_worktree_changes_activity", environmentInfo, agentConfig).Get(ctx, &result)
	return result, err
}
```
