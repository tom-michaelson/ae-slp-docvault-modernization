...
name: jira-operator
description: |-
  Use this agent when you need to interact with Jira issues through the Atlassian MCP server including reading issue details, updating issue fields, adding comments, transitioning issue status, or any other Jira-related operations. This agent uses the Atlassian MCP tools for seamless integration.

  Examples:
  <example>
  Context: The user needs to retrieve details about a specific Jira issue.
  user: "Get the details for issue ABC-123"
  assistant: "I'll use the jira-operator agent to retrieve the issue details for you."
  <commentary>
  Since the user is requesting Jira issue information, use the Task tool to launch the jira-operator agent to fetch and format the issue details.
  </commentary>
  </example>
  <example>
  Context: The user wants to add a comment to a Jira issue after completing some work.
  user: "Add a comment to DEF-456 saying the database migration is complete"
  assistant: "I'll use the jira-operator agent to add that comment to the issue."
  <commentary>
  Since the user wants to update a Jira issue with a comment, use the Task tool to launch the jira-operator agent to add the comment.
  </commentary>
  </example>
  <example>
  Context: The user needs to update multiple fields on a Jira issue.
  user: "Update issue GHI-789 to set status to 'In Progress' and assign it to john.doe"
  assistant: "I'll use the jira-operator agent to update those issue fields."
  <commentary>
  Since the user wants to modify Jira issue fields, use the Task tool to launch the jira-operator agent to perform the updates.
  </commentary>
  </example>
color: blue
claude:
