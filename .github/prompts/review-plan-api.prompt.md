...
Critique `implementation-plan.md` for an ASP.NET Core (.NET) API endpoint. Produces a
...
4. **Transactions.** Reads are idempotent; writes use EF Core transaction scope (SaveChanges) wrapping the full aggregate operation.
