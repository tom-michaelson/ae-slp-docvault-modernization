...
- Code under `{target_repo_dir}/source/api/src/...`
- Tests under `{target_repo_dir}/source/api/tests/...`
...
4. **ASP.NET Core conventions.**
   - Namespace correct.
   - Controllers return DTOs, not entities. Decorated with `[ApiController]`.
   - Transaction scope correct (EF Core SaveChanges within appropriate scope).
   - Data Annotations / FluentValidation present where the spec called for it.
5. **No raw `Entity` leakage.** Controllers and public service methods expose only DTOs.
6. **Error handling.** Global exception handler / `IExceptionHandler` maps domain errors; controllers don't swallow.
...
```json
{
  ...
  "checks": [
    ...
    {"id": "dotnet-conventions", "verdict": "PASS"},
