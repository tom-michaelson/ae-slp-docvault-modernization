...
You need both:
- `legacyPath` — relative path on `localhost:5106` (e.g. `/Basket`, `/`)
- `newPath` — relative path on `localhost:4200` (e.g. `/basket`, `/`)
...
- **Form-submission flows**: `flow-{action-slug}` (e.g. `flow-update`,
  `flow-add-to-basket`)
...
```json
{
  ...
  "screenshots": [
    ...
    },
    {
      "name": "empty-state",
      "category": "baseline",
      "description": "Page rendered when the user has no data yet (e.g. empty basket).",
    ...
    },
    {
      "name": "flow-update",
      "category": "flow",
      "description": "After the user submits an update form (post-submit state).",
      "navigationHint": "Change a quantity and click [ UPDATE ].",
      "expectedElements": ["updated row values", "recomputed totals"]
    }
  ]
}
```
