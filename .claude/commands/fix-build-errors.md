...
step after `./gradlew clean build -x test` (API) or `npm run build` (UI) fails.
...
- Source under `{target_repo_dir}/source/api/` (Java) and
...
2. For each:
   ...
   - Understand the error category (missing import, type mismatch, missing
     dependency, Lombok annotation misuse, Angular template type error, etc.).
...
- [ ] After your edits, `./gradlew clean build -x test` (API) and/or
