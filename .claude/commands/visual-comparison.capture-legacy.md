...
**legacy** application (`http://localhost:5106` by default). No login, no menu
tree — direct URL navigation.

## Arguments

- `$0` — page key (folder under `docs/entry-points/ui-pages/`). Examples: `home`, `basket`.
...
```
mcp__playwright__browser_navigate(url="http://localhost:5106<legacyPath>")
mcp__playwright__browser_wait_for(time=2)
mcp__playwright__browser_snapshot()
```

The legacy app is plain server-rendered Razor — no SPA wait beyond the initial
...
- **Home**: navigate to `/` — already populated from `data.sql`. Nothing to do.
- **Basket**: a fresh anonymous session has an empty basket. To capture
  populated baselines, first add 2 items via the home page:
  ```
  mcp__playwright__browser_navigate(url="http://localhost:5106/")
  mcp__playwright__browser_snapshot()
  # Click two of the [ ADD TO BASKET ] buttons (they POST a form to /Basket)
  mcp__playwright__browser_click(element="first [ ADD TO BASKET ] submit", ref=<ref>)
  mcp__playwright__browser_navigate(url="http://localhost:5106/")
  mcp__playwright__browser_snapshot()
  mcp__playwright__browser_click(element="third [ ADD TO BASKET ] submit", ref=<ref>)
  mcp__playwright__browser_navigate(url="http://localhost:5106<legacyPath>")
  ```
