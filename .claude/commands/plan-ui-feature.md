...
| `target_stack` | Typically `angular-java` |
...
- **Forms** — reactive form schema + validators (Bean-Validation-equivalent).
- **Styling** — which brand tokens (`--eshop-teal`, `--eshop-green`, etc.)
  apply; new SCSS files only if the feature needs non-layout styling.
...
- Endpoint refs: `list-catalog-items`, etc. (from `api-usage.json`).
...
| Path | Action |
| --- | --- |
| `target_repo/source/ui/src/app/pages/home/home.component.ts` | modify |
| `target_repo/source/ui/src/app/pages/home/catalog.service.ts` | create |
| ... | ... |
```
