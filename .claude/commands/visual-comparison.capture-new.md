...
- **Home**: data is fetched from `/api/catalog-items` — already populated.
  Nothing to seed.
- **Basket**: state lives in `localStorage.eshop.buyerId` plus the API. To
  capture populated baselines, first add items by clicking `[ ADD TO BASKET ]`
  on the home page (the click fires `BasketService.addItem` which POSTs to
  `/api/baskets/for-user/{buyerId}/items`):
  ```
  mcp__playwright__browser_navigate(url="http://localhost:4200/")
  mcp__playwright__browser_snapshot()
  mcp__playwright__browser_click(element="first [ ADD TO BASKET ]", ref=<ref>)
  mcp__playwright__browser_click(element="third [ ADD TO BASKET ]", ref=<ref>)
  mcp__playwright__browser_navigate(url="http://localhost:4200<newPath>")
  ```

If the capture plan includes `empty-state`, capture it FIRST in a fresh
browser context (clears localStorage so `BasketService` mints a new buyer-id
and gets an empty basket), then seed and capture the populated baseline.
...
- The `BasketService` stores a UUID in `localStorage` keyed `eshop.buyerId`. A
  brand-new browser context gets a fresh UUID and an empty basket — handy for
  capturing the empty-state baseline cleanly.
- The new app's basket page lives at `/basket` (lowercase), distinct from the
  legacy `/Basket` (capital B). The capture plan tracks them separately as
  `newPath` and `legacyPath`.
