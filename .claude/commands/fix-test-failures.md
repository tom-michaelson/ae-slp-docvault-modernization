...
`./gradlew test` (API) or `npm test` (UI) fails.
...
- JUnit XML (if present) under
  `{target_repo_dir}/source/api/build/test-results/test/*.xml`.
...
- Never `@Disabled` / `xit()` your way to green.
...
- [ ] `./gradlew test` (API) and/or `npm test -- --watch=false` (UI) pass.
